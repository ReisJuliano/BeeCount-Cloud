...
- **13 个 read tool**:`list_ledgers` / `list_transactions` / `list_categories` / `list_accounts` / `list_tags` / `list_budgets` / `get_ledger_stats` / `get_analytics_summary` / `search` / `get_transaction` / `get_active_ledger` / `get_account_balances` / `lookup_merchant_category`
- **8 个 write tool**:`create_transaction` / `create_transactions`(批量导入,一次提交多笔)/ `update_transaction` / `delete_transaction`(需二次确认)/ `create_category` / `update_budget` / `parse_and_create_from_text`(让 BeeCount AI 解析自然语言)/ `set_merchant_category`
...
| `get_account_balances` | 每个账户当前余额 | — |
| `lookup_merchant_category` | 查某个收款方之前学到的分类 | merchant_name |
...
| `set_merchant_category` | 记住某个收款方对应的分类 | merchant_name, category_name |
